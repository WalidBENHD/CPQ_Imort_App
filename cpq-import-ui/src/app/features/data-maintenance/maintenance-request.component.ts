...
type MaintenanceAction = 'withdraw' | 'approve' | 'return' | 'publish' | null;
...
@Component({
  ...
  template: `
    <section class="request-page">
      <header class="request-hero">
        <div>
          <a routerLink="/maintenance"><mat-icon>arrow_back</mat-icon> Data maintenance</a>
      ...
      <ng-container *ngIf="!loading && !loadError">
        ...
        <section class="decision-bar">
          ...
          <div class="actions">
            <button mat-stroked-button class="workflow-action" [class.workflow-action--running]="workflowAction === 'withdraw'" *ngIf="canWithdraw" [disabled]="acting" [attr.aria-busy]="workflowAction === 'withdraw'" (click)="withdraw()">
              <app-publication-progress *ngIf="workflowAction === 'withdraw'; else withdrawLabel" label="Restoring workspace" />
              <ng-template #withdrawLabel><mat-icon>undo</mat-icon>Withdraw</ng-template>
            </button>
            ...
            <button mat-flat-button class="approve workflow-action" [class.workflow-action--running]="workflowAction === 'approve'" *ngIf="canDecide" [disabled]="acting" [attr.aria-busy]="workflowAction === 'approve'" (click)="approve()">
              <app-publication-progress *ngIf="workflowAction === 'approve'; else approveLabel" label="Approving change set" />
              <ng-template #approveLabel><mat-icon>verified</mat-icon>Approve change set</ng-template>
            </button>
            <button mat-flat-button class="publish" [class.publish--running]="workflowAction === 'publish'" *ngIf="canPublish" [disabled]="acting" [attr.aria-busy]="workflowAction === 'publish'" (click)="publish()">
              <app-publication-progress *ngIf="workflowAction === 'publish'; else maintenancePublishLabel" label="Publishing changes" />
              <ng-template #maintenancePublishLabel><mat-icon>rocket_launch</mat-icon>Publish changes</ng-template>
            </button>
          </div>
        </section>

        <section class="workflow-evidence" aria-label="Request responsibility history">
          <header><span class="section-kicker">Governance evidence</span><h2>People behind this change</h2></header>
          <div>
            <article *ngFor="let evidence of workflowEvidence">
              <mat-icon>{{ evidence.icon }}</mat-icon>
              <span><small>{{ evidence.label }}</small><strong>{{ evidence.actor }}</strong><em>{{ evidence.occurredAt | date:'dd MMM yyyy, HH:mm' }}</em></span>
            </article>
          </div>
        </section>
        ...
        <section class="decision-panel" *ngIf="showDecision">
          <div class="decision-dialog">
            ...
            <footer><button mat-button (click)="showDecision = false">Cancel</button><button mat-flat-button color="warn" class="workflow-action" [class.workflow-action--running]="workflowAction === 'return'" [disabled]="!decisionReason.trim() || acting" [attr.aria-busy]="workflowAction === 'return'" (click)="returnForCorrection()"><app-publication-progress *ngIf="workflowAction === 'return'; else returnRequestLabel" label="Returning request" /><ng-template #returnRequestLabel>Return request</ng-template></button></footer>
          </div>
        </section>
  ...
  `, `
    ...
    @media(max-width:800px){.validation-errors article{grid-template-columns:1fr}}
  `]
})
export class MaintenanceRequestComponent implements OnInit {
  ...
  loadError = false;
  acting = false;
  workflowAction: MaintenanceAction = null;
  showDecision = false;
  decisionReason = '';
  ...
  get workflowEvidence(): WorkflowEvidenceItem[] {
    const job = this.items[0]?.job;
    const candidates: Array<WorkflowEvidenceItem | null> = [
      this.evidence('Prepared by', 'edit_note', this.ownerName, this.createdAt),
      this.evidence('Submitted by', 'send', this.package?.submittedByDisplayName ?? job?.submittedByDisplayName, this.package?.submittedAt ?? job?.submittedAt),
      this.evidence('Approved by', 'verified', this.package?.approvedByDisplayName ?? job?.approvedByDisplayName, this.package?.approvedAt ?? job?.approvedAt),
      this.evidence('Published by', 'rocket_launch', this.package?.publishedByDisplayName ?? job?.committedBy, this.package?.publishedAt ?? job?.committedAt)
    ];
    return candidates.filter((item): item is WorkflowEvidenceItem => item !== null);
  }
  ...
  load(): void {
    ...
    source.subscribe({
      next: value => {
        const isPrivate = this.kind === 'package'
          ? (value as ReleasePackage).status === 0
          : (value as ImportJob).workflowStageLabel === 'Private';
        if (isPrivate) {
          this.loading = false;
          this.toast.warning('Private maintenance changes are prepared and validated in the maintenance form.');
        ...
        forkJoin(jobIds.map(jobId => forkJoin({
          ...
          approvalSnapshot: this.imports.getApprovalSnapshot(jobId),
          errors: this.imports.getRows(jobId, 1, 200, null, 'Error')
        })))
          .pipe(finalize(() => this.loading = false))
          .subscribe({
            next: items => this.items = items.map(item => ({
              job: item.job,
              comparison: item.approvalSnapshot?.comparison ?? item.comparison,
              errorRows: item.errors.items
            })),
  ...
  approve(): void { this.run(this.package ? this.imports.approveReleasePackage(this.id) : this.imports.approve(this.id), 'Maintenance request approved.', 'approve'); }
  publish(): void { this.run(this.package ? this.imports.publishReleasePackage(this.id) : this.imports.publish(this.id), 'Maintenance changes published.', 'publish'); }
  returnForCorrection(): void {
    ...
    this.run(action, 'Maintenance request returned with correction guidance.', 'return');
  }

  private run(action: Observable<unknown>, success: string, workflowAction: MaintenanceAction = null): void {
    this.acting = true;
  ...
  private evidence(label: string, icon: string, actor: string | null | undefined, occurredAt: string | null | undefined): WorkflowEvidenceItem | null {
    return actor && occurredAt ? { label, icon, actor, occurredAt } : null;
  }
}
