...
import { DependencyContext, DependencyImpact, ImportJob, ReleasePackage } from '../../core/models/import.models';
      ...
      <ng-container *ngIf="!loading && context as ctx">
        ...
        <section class="release-setup" *ngIf="ctx.isDependentDataset && view === 'release' && !ctx.releasePackage">
          ...
          <div class="setup-grid">
            <mat-form-field appearance="outline"><mat-label>Release name</mat-label><input matInput [(ngModel)]="releaseName" maxlength="180"></mat-form-field>
            <mat-form-field appearance="outline"><mat-label>Article Master candidate</mat-label><mat-select [(ngModel)]="selectedCandidateId"><mat-option *ngFor="let master of ctx.candidateMasters" [value]="master.jobId">{{ master.fileName }} · {{ master.articleCount }} articles</mat-option></mat-select></mat-form-field>
          </div>
          <div class="empty-candidates" *ngIf="!ctx.candidateMasters.length"><mat-icon>upload_file</mat-icon><span>No error-free private Article Master is available. Upload and prepare the master first.</span></div>
          <div class="notice"><mat-icon>verified_user</mat-icon><div><strong>The candidate becomes this draft's validation anchor.</strong><span>Both uploads remain private until you submit the complete release package.</span></div></div>
        ...
        <section class="release-board" *ngIf="ctx.releasePackage as release">
          ...
          <div class="actions package-actions">
            ...
            <button *ngIf="canSubmit(release)" mat-flat-button color="primary" [disabled]="busy || packageHasErrors(release)" (click)="submitPackage(release)"><mat-icon>send</mat-icon> Submit complete release</button>
...
export class DependencyContextPrototypeComponent implements OnChanges {
  ...
  load(): void {
    ...
    this.imports.getDependencyContext(this.jobId).subscribe({
      next: context => { this.context = context; this.loading = false; this.selectedCandidateId ||= context.candidateMasters[0]?.jobId ?? ''; },
  ...
  releaseStatusLabel(status: number): string { return ['Draft','In review','Approved','Publishing','Published','Action required'][status] ?? 'Unknown'; }
