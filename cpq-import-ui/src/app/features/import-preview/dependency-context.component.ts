...
type PackageAction = 'submit' | 'approve' | 'reject' | 'publish' | 'dissolve' | null;

@Component({
  selector: 'app-dependency-context-prototype',
  standalone: true,
  imports: [CommonModule, FormsModule, MatButtonModule, MatFormFieldModule, MatIconModule, MatInputModule, MatSnackBarModule, PublicationProgressComponent],
  template: `
        ...
        <section class="release-board" *ngIf="ctx.releasePackage as release">
          ...
          <div class="release-sequence">
            <ng-container *ngFor="let item of release.items; let index = index">
              <article
                class="release-item"
                [class.current]="item.jobId === jobId"
                [class.clickable]="item.jobId !== jobId"
                [attr.role]="item.jobId !== jobId ? 'button' : null"
                [attr.tabindex]="item.jobId !== jobId ? 0 : null"
                [attr.aria-label]="item.jobId !== jobId ? 'Open ' + item.fileName : null"
                (click)="openReleaseItem(item.jobId)"
                (keydown.enter)="openReleaseItem(item.jobId)"
                (keydown.space)="openReleaseItem(item.jobId); $event.preventDefault()">
          ...
          <div class="actions package-actions">
            ...
            <button *ngIf="canDissolve(release)" mat-stroked-button class="dissolve-package package-action" [class.package-action--running]="packageAction === 'dissolve'" [disabled]="busy" [attr.aria-busy]="packageAction === 'dissolve'" (click)="dissolvePackage(release)">
              <app-publication-progress *ngIf="packageAction === 'dissolve'; else dissolveLabel" label="Dissolving release" />
              <ng-template #dissolveLabel><mat-icon>link_off</mat-icon> Dissolve release</ng-template>
            </button>
            <button *ngIf="canSubmit(release)" mat-flat-button color="primary" class="package-action" [class.package-action--running]="packageAction === 'submit'" [disabled]="busy || packageHasErrors(release) || !packageIsPortfolioReady" [attr.aria-busy]="packageAction === 'submit'" (click)="submitPackage(release)">
              <app-publication-progress *ngIf="packageAction === 'submit'; else submitLabel" label="Submitting release" />
              <ng-template #submitLabel><mat-icon>send</mat-icon> {{ packageIsPortfolioReady ? 'Submit complete release' : 'Complete release pair' }}</ng-template>
            </button>
            <button *ngIf="canApprove(release)" mat-flat-button color="primary" class="package-action" [class.package-action--running]="packageAction === 'approve'" [disabled]="busy" [attr.aria-busy]="packageAction === 'approve'" (click)="approvePackage(release)">
              <app-publication-progress *ngIf="packageAction === 'approve'; else approveLabel" label="Approving release" />
              <ng-template #approveLabel><mat-icon>verified</mat-icon> Approve release</ng-template>
            </button>
            ...
            <button
              *ngIf="canPublish(release)"
              mat-flat-button
              color="primary"
              class="package-publish"
              [class.package-publish--running]="packageAction === 'publish'"
              [disabled]="busy"
              [attr.aria-busy]="packageAction === 'publish'"
              (click)="publishPackage(release)">
              <app-publication-progress *ngIf="packageAction === 'publish'; else publishPackageLabel" label="Publishing release" />
              <ng-template #publishPackageLabel><mat-icon>rocket_launch</mat-icon> {{ release.status === 5 ? 'Resume publication' : 'Publish complete release' }}</ng-template>
            </button>
          </div>
          <div class="release-reject-panel" *ngIf="showReleaseRejectPanel && canReject(release)">
            ...
            <div class="release-reject-actions">
              <button mat-flat-button color="warn" class="package-action" [class.package-action--running]="packageAction === 'reject'" [disabled]="busy || !releaseRejectionReason.trim()" [attr.aria-busy]="packageAction === 'reject'" (click)="rejectPackage(release)">
                <app-publication-progress *ngIf="packageAction === 'reject'; else rejectLabel" label="Returning release" />
                <ng-template #rejectLabel>Confirm rejection</ng-template>
              </button>
  ...
  styles: [`
    ...
    .package-action--running:not(.dissolve-package){color:#fff!important}.dissolve-package.package-action--running{color:#b91c1c!important}
...
export class DependencyContextPrototypeComponent implements OnChanges {
  ...
  loading = false;
  busy = false;
  packageAction: PackageAction = null;
  errorMessage = '';
  selectedCandidateId = '';
  candidateSearch = '';
  ...
  submitPackage(release: ReleasePackage): void { this.runPackageAction(this.imports.submitReleasePackage(release.id), 'Release submitted for approval.', 'submit'); }
  approvePackage(release: ReleasePackage): void { this.runPackageAction(this.imports.approveReleasePackage(release.id), 'Release approved with immutable evidence.', 'approve'); }
  publishPackage(release: ReleasePackage): void { this.runPackageAction(this.imports.publishReleasePackage(release.id), 'Release published in dependency order.', 'publish'); }
  ...
  rejectPackage(release: ReleasePackage): void {
    ...
    this.imports.rejectReleasePackage(release.id, reason).subscribe({
      ...
      error: error => {
        this.packageAction = null;
        this.fail(error, 'The release could not be rejected.');
      }
    });
  }
  ...
  openReleaseItem(itemJobId: string): void {
    if (itemJobId !== this.jobId) this.router.navigate(['/import', itemJobId]);
  }
  ...
  dissolvePackage(release: ReleasePackage): void {
    ...
    this.imports.dissolveReleasePackage(release.id).subscribe({
      ...
      error: error => {
        this.packageAction = null;
        this.fail(error, 'The release could not be dissolved.');
      }
    });
  }

  private runPackageAction(request: ReturnType<ImportService['submitReleasePackage']>, successMessage: string, action: Exclude<PackageAction, null>): void {
    this.busy = true;
    ...
    request.subscribe({
      ...
      error: error => {
        this.packageAction = null;
        this.fail(error, 'The release action could not be completed.');
      }
    });
  }
